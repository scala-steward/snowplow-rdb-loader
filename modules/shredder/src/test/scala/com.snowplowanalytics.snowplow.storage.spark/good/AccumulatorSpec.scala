...
class AccumulatorSpec extends Specification with ShredJobSpec {
  ...
  "A SqsMessage" should {
    "encoded as JSON and then decoded should return the same message" in {
      val orig = SqsMessage(
        List(
          ShreddedType(SchemaKey("org.schema", "WebPage" , "jsonschema" , SchemaVer.Full(1,0,0)), "/shredded-tsv/vendor=org.schema/name=WebPage/format=jsonschema/version=1", ShreddedFormat.Tsv),
          ShreddedType(SchemaKey("com.snowplowanalytics.snowplow", "atomic", "jsonschema", SchemaVer.Full(1,0,0)), "/atomic-events", ShreddedFormat.Tsv),
          ShreddedType(SchemaKey("com.snowplowanalytics.snowplow", "link_click", "jsonschema", SchemaVer.Full(1,0,0)), "/shredded-tsv/vendor=com.snowplowanalytics.snowplow/name=link_click/format=jsonschema/version=1", ShreddedFormat.Tsv)
        )
      )
      SqsMessage.parse(orig.compact) ==== Right(orig)
    }
  }
}