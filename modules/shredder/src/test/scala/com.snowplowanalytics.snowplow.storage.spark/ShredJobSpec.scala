...
trait ShredJobSpec extends SparkSpec {
   ...
   */
  def runShredJob(lines: Lines, crossBatchDedupe: Boolean = false): (String, Set[ShreddedType]) = {
    ...
    (dirs.output.toString, result)
  }

  def runShredJobTabular(lines: Lines, crossBatchDedupe: Boolean = false, tabularBlacklist: List[SchemaKey] = Nil): (String, Set[ShreddedType]) = {
    val input = mkTmpFile("input", createParents = true, containing = lines.some)
    val config = Array(
      "--input-folder", input.toString(),
      "--output-folder", dirs.output.toString(),
  ...
  def storageConfig(tabularBlacklist: List[SchemaKey]) = {
    val encoder = new Base64(true)
    val blacklist =
      tabularBlacklist
